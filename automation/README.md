...
| TW Creative pipeline sync | `trig_01Bz5iWJZ9cBkXjhd2YeCaF4` (fresh session, paused until connectors attached) · old session-bound `trig_011EhN2rJaHcNoAKHPAY8it3` | Daily 09:00 & 15:00 | Ideas (Status=In progress) → Roadmap rows (Plan); Roadmap (Ready-to-Test+) → Ideas marked Done; Ready-to-Test Slack ping tagging Kevin; design-done hints |
| TW Creative Roadmap nightly sync | `trig_01Mu2vcfPW9zXbT9N1s9nXn8` (fresh session, paused until connectors attached) · old session-bound `trig_01EZ44tzBdZfTfJy1scvf128` | Daily 10:00 | Meta live ads + BigQuery SP scores → Roadmap (SP score/status, CPFT, spend, Last synced); creates rows for unknown live ads; pause detection + owner pings; Hit-ad → #hit-ads-library; watch flags; archive hygiene |
| TW winning creative iteration analyst | `trig_01NJznSip628rN38F1f1jHAL` (fresh session, paused until connectors attached) · old session-bound `trig_01JqGLRu34KTW38yFpWNpDDt` | Mon 10:30 | Hit Ad / P2 Loser rows → Motion analysis → iteration brief → "Winning Iteration" idea in Ideas board (cap 5/week) |
...
**Architecture note (important):** the three routines were originally created from inside a session and fire *into the persistent automation session* (`session_014szRHcYSqZhSCd695DuyA9`). Verified 2026-09-03: routines created from inside a session — whether session-bound or fresh-session — run in `auto` permission mode and stop at *Requires action* on connector calls; they only progress when a human approves. Routines created from the **claude.ai Routines UI** run with no permission prompts and finish unattended, and can hold connector grants. The fix is to recreate each routine in the UI by pasting the prompt from [`routines/trigger_prompts.md`](routines/trigger_prompts.md) (which also lists connectors per routine, the trigger IDs to retire, and the cutover procedure). Until that is done the session-bound triggers stay enabled; the agent-minted fresh-session triggers are paused.
