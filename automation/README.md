...
**Architecture note (important):** the three routines were originally created from inside a session and fire *into the persistent automation session* (`session_014szRHcYSqZhSCd695DuyA9`) because agent-created triggers cannot carry MCP connector grants — fresh sessions they spawn have no Notion/Slack/Meta/BigQuery tools (verified 2026-08-16, and again 2026-09-03: `create_trigger` rejects the `connectors` parameter for this org). On 2026-09-03 fresh-session ("auto") replacements were created, **paused**, with standalone prompts; see [`routines/trigger_prompts.md`](routines/trigger_prompts.md) for the prompts, the connector list per routine, and the cutover procedure (attach connectors in the Routines UI → DRY RUN → enable new / disable old). Until that cutover is done the session-bound triggers keep running.
