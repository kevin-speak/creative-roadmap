...
**Switching to set B:** `fire_trigger` the nightly trigger with the text `DRY RUN …` (the prompt has a dry-run branch that only makes read-only calls). If the fired session ends idle with all four connectors reported reachable, enable set B (`update_trigger enabled: true` ×3) and disable set A (`enabled: false` ×3). If the fired session ends in *requires action* (a permission prompt), the fresh-session path is still blocked by the auto-mode classifier for connector tools; keep set A and approve its prompts in the app.
